...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in file.remove("{}analysis-post-2008-CHUNKTIMINGS.txt"{}): cannot remove file 'analysis-post-2008-CHUNKTIMINGS.txt', reason 'No such file or directory'}}\begin{verbatim}
## [1] FALSE
\end{verbatim}
\begin{alltt}
...
\hlstd{cancellations.by.year.carrier.other} \hlkwb{<-}
  ...
  \hlkwd{group_by}\hlstd{(Year,} \hlkwc{Carrier_other} \hlstd{=} \hlkwd{ifelse}\hlstd{(UniqueCarrier} \hlopt{%in%} \hlstd{biggest.carriers, UniqueCarrier,} \hlstr{"Other"}\hlstd{))} \hlopt{%>%}
...
\hlstd{cancellations.by.year.carrier.other} \hlopt{%>%}
  \hlstd{tbl_df} \hlopt{%>%}
  \hlkwd{mutate}\hlstd{(}\hlkwc{Carrier_other} \hlstd{=} \hlkwd{factor}\hlstd{(Carrier_other,} \hlkwc{levels} \hlstd{=} \hlkwd{c}\hlstd{(biggest.carriers,} \hlstr{"Other"}\hlstd{)))} \hlopt{%>%}
  \hlkwd{arrange}\hlstd{(Year, Carrier_other)} \hlopt{%>%}
  \hlkwd{ggplot}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= Year,} \hlkwc{y} \hlstd{= total_cancellations,} \hlkwc{fill} \hlstd{= Carrier_other))} \hlopt{+}
  \hlkwd{geom_area}\hlstd{()} \hlopt{+}
  \hlkwd{guides}\hlstd{(}\hlkwc{fill} \hlstd{=} \hlkwd{guide_legend}\hlstd{(}\hlkwc{reverse} \hlstd{=} \hlnum{TRUE}\hlstd{))} \hlopt{+}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'Year' not found}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'expected.cancellations.by.week' not found}}\end{kframe}
\end{knitrout}
...
\hlstd{flights.by.origin} \hlkwb{<-}
  \hlkwd{count}\hlstd{(flights, Origin)} \hlopt{%>%}
  \hlkwd{arrange}\hlstd{(}\hlkwd{desc}\hlstd{(n))}

\hlstd{flights.by.airport.carrier} \hlkwb{<-}
  \hlstd{flights} \hlopt{%>%}
  \hlkwd{count}\hlstd{(Origin, UniqueCarrier)} \hlopt{%>%}
  \hlkwd{arrange}\hlstd{(}\hlkwd{desc}\hlstd{(n))}
...
\begin{verbatim}
##              used    (Mb) gc trigger    (Mb)   max used    (Mb)
## Ncells   66830879  3569.2   90165604  4815.4   66830879  3569.2
## Vcells 2717880664 20735.8 6076621632 46361.0 2717880664 20735.8
\end{verbatim}
\end{kframe}
\end{knitrout}
...
COMPILATION TIME: 23.1922272324562
\end{document}
