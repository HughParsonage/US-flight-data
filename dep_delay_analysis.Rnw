...
Sys.time()
flights <- fread("../1987-2015-On-Time-Performance.csv", select = c("Year", "Month", "DayofMonth", "DepTime", "ArrTime", "UniqueCarrier",
                                                                    "ActualElapsedTime", "ArrDelay", "DepDelay", "Origin", "Dest", "Cancelled"))
...
flights_hubs %>%
  ...
  ggplot(aes(x = Date, y = n * sample.weight.int, color = Origin, group = Origin)) + 
  geom_line() + 
  geom_point()
@
